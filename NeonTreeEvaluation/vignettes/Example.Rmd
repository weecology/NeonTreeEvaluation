---
title: "Example Predictions and Evaluation"
...
xml_polygons <- xml_to_spatial_polygons(xmls[xmls$filename %in% "SJER_052.tif",],rgb)
...
```{r}
#Use utility function to turn into dataframes
predictions<-bind_rows(lapply(bboxes@polygons,extract_coordinates))
```
...
statdf<-calc_jaccard(assignment=assignment,ground_truth = ground_truth,prediction=predictions[[i]]) %>% mutate(Method=names(predictions)[i])
```
